...
$(window).resize(function () {
	...
	$('canvas').attr('id','imageMap').attr('height', $(window).height() - 37).attr('width', $(window).width() - 232);
	...
	if ($('#cropoverlay').css('display') == 'block') {
		$('#cropoverlay').css({ 
			left: Math.ceil(app.canvas.width / 2 - app.getActiveLayer().x - app.getActiveLayer().regX - 60) + 'px', 
			top: Math.ceil(app.canvas.height / 1 + app.getActiveLayer().y - app.getActiveLayer().regY - 10) + 'px'
		});
	}
});
$('body').on('click','.imageupload',function(){
  var img = $(this).attr('src');
  $('#imageMap').css("background-image", "url(" + img + ")");
 /*var imgArray = $(this);
 ...
 function convertImageToCanvas(image) {
 	 var prevcanvas = document.getElementById("imageMap");
     var canvas = document.createElement("canvas");
     canvas.width = canvas.width;
     canvas.height = canvas.height;
     canvas.getContext("2d").drawImage(image, 0, 0);
     $(prevcanvas).replaceWith(canvas);
      // $("#imageMap").replaceWith(canvas);
...
$(document).ready(function () {
	...
	$('#button-filterbrightness').click(function () {
		...
		$('#dialog-filterbrightness input').val('50');
	});
	...
	$('#button-openfile').change(app.callbacks.openFile);
